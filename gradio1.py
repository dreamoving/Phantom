        ...
        if 1:
            ...
            if sr_model == 'PASD':
                ...
                try:
                    ...
                    # ocr
                    result = ocr_detection(f'output/{timename}_seed{seed}_origin.png')
                    bbox_list = result['polygons']
                    if len(bbox_list) > 0 and mos_score >= 0.4:
                        print(bbox_list)
                        merge_flag = True
            ...
            if sr_model == 'GAN_2x':
                try:
                    # GAN
                    input_image_GAN = np.array(input_org_image)[:, :, ::-1] # convert to numpy BGR
                    image_GAN, _ = upsampler_2x.enhance(input_image_GAN, outscale=2)
                    image_GAN = Image.fromarray(image_GAN[:, :, ::-1]) # convert back
                except Exception as e:
                    print(e)
                    image_GAN = Image.new(mode="RGB", size=(ori_width*upscale, ori_height*upscale))
            ...
            if sr_model == 'SwinIR' or merge_flag:
                try:
                    # SwinIR
            ...
            if merge_flag:
                ...
                for bbox in bbox_list:
                    x_low = max(min(bbox[0], bbox[6]) * upscale, 4)
                    x_high = min(max(bbox[2], bbox[4]) * upscale, out_w-4)
                    y_low = max(min(bbox[1], bbox[3]) * upscale, 4)
                    y_high = min(max(bbox[5], bbox[7]) * upscale, out_h-4)
                    bbox = [x_low-4,y_low-4,x_high+4,y_high+4]
                    
                    img1_crop = img1[bbox[1]:bbox[3], bbox[0]:bbox[2]]
                    img2_crop = img2[bbox[1]:bbox[3], bbox[0]:bbox[2]]
                    img_transferred = rm.transfer_color(img1_crop, img2_crop, "meanstd")
                    img2_merged = rm.merge(img_transferred, img2_merged, [bbox[0],bbox[1],bbox[2]-bbox[0],bbox[3]-bbox[1]], 'smooth_cleanedge')
                
                image = Image.fromarray(img2_merged)
